...
**커스텀 커맨드는 스킬로 합쳐졌다.** 기본값은 *사용자와 Claude 둘 다*이고,
`disable-model-invocation: true` 로 **사용자 전용**이 된다.
...
| 값 | 무슨 일이 |
|---|---|
| **`auto`**(기본) | 설치된 것에서 고른다. 없으면 내부로 떨어지고 **그 사실을 말한다** |
| `codex` | `/codex:review` · `/codex:adversarial-review` |
| 그 밖의 이름 | 그 도구의 리뷰 명령. 없으면 diff 를 주고 **2축**(Standards·Spec)으로 묻는다 |
| `none` | 🔶 내부로 떨어진다. **제3자성이 없다는 것을 사용자에게 말한다** |

### 🔴 사다리는 셋이고 기본은 **가장 낮은 칸**이다

| 칸 | 무엇 | 언제 |
|---|---|---|
| **1 · 수동**(`/review`) | 부를 때만 돈다 | **기본.** 비용이 예측 가능 |
| 2 · **Stop 게이트** | 턴이 끝나는 것을 막는다 | 🔶 **옵트인** — `codex` 의 `--enable-review-gate` |
| **3 · 벽**(CI) | 푸시마다. 빨간불이면 머지 불가 | 🔒 **유일한 진짜 집행** |

**2칸을 기본으로 켜지 않는다.** 실사용 보고가 일관된다 — *"수동부터 시작하라"* ·
*"게이트는 **비용 폭주**를 만들 수 있다"*(수정 → 재리뷰 → 재수정). OpenAI 문서 자신이
*"**직접 감독할 때만** 켜라"* 고 적는다.

🔵 **되먹임 루프는 에이전트 *안쪽* 게이트에만 생긴다** — 벽은 **푸시마다 한 번** 돌아 같은 루프를 못 만든다.
