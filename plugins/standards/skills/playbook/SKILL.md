...
**산출물의 인수 강제는 GitHub 벽만 한다. 이 지도는 전부 "이렇게 하면 좋아요" 다.**
(로컬 실행 피해 — 비밀 읽기 · force push · `rm -rf` — 는 템플릿의 `.claude/settings.json` deny 가 막는다. 벽은 그걸 되돌려주지 못한다.)
기획·스펙·티켓·구현·리뷰 같은 **작업 스킬은 `mattpocock-skills` 를 재사용**한다 — 우리가 검토하고 핀한 외부 의존성이다. 우리가 만든 건 **진입(이 지도)과 문(`/new-project` · `/floor-check`)** 뿐이다.
...
| 4 | 만들기 (티켓마다) | `/clear` → 브랜치 → `/implement #N` → **draft PR** → `gh pr ready` | tdd 로 구현 · 자기 code-review · 커밋 → CI 가 돈다 → ready HEAD 를 제3자가 본다 | **구현 방법은 에이전트가.** 범위 · 위험 · 비용 · AC 가 달라지면 사람이 — 그리고 머지 버튼 | CI 초록 · 제3자 리뷰 · 머지 |
...
**고치는 자리는 draft, 확인하는 자리는 ready.** 리뷰 단위는 PR 이 아니라 **ready 상태의 HEAD** 다 —
draft 에서는 0회, ready HEAD 마다 1회. ready 뒤에 푸시하면 그 HEAD 는 새 리뷰 대상이다(보통 1~2회, 29회가 아니다).
